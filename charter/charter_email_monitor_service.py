...
IMAP_USER = "opentonnages@hifleet.com"
IMAP_PASSWORD = "yzIcMMo4tD0kuolJ"
...
from charter_utils import sanitize_date, generate_vessel_tags, extract_reply, desensitize_text, convert_date, \
    ...
    generate_cargo_tags, Sm4Util, call_deepseek_region, load_imo_email_records_from_xlsx, get_portid, \
    classify_vessel_type, call_deepseek_shiptype
...
log_file = r'E:\xwc\vessel_sales_purchase\logs\charter_email_monitor_logfile.log'
                    ...
                    try:
                        ...
                        if "openvessels" in intent_list and "openvessels" in charter_data:
                            ...
                            for vessel in charter_data["openvessels"]:
                                ...
                                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0, 1,%s,%s,%s,%s,%s,%s,%s,%s,%s
                            )
                                """
                                cursor.execute(insert_open_sql, insert_values)
                                vessel_id = cursor.lastrowid
                                if is_owner == 1:
                                    dataIdEncode = Sm4Util.encrypt(vessel_id)
                                    url = "http://ttse.hifleet.com/shipdetail/point/share-data/reward"
                                    payload = {
                                        "dataIdEncode": dataIdEncode,
                                        "userId": user_id,
                                        "dataId": vessel_id,
                                        "point": 245
                                    }
                                    ...
                                    try:
                                        ...
                                        print("调用点数增加接口出错")
                                        logger.error(f"调用点数增加接口出错:{payload} - {e}")
                                # # 3. 获取在匹配区间内的货盘记录
                                # sql_cargo = f"""
    ...
    update_sql = """
        ...
        WHERE id NOT IN (
            ...
            FROM (
                ...
                GROUP BY email_users, user_id, vessel_name, open_port,  sender_name, sender_email, email_body
            ) AS t
        )
        AND is_duplicate = 0
    """
